...
This script should be run before importing Pyrogram.
"""
...
def patch_pyrogram():
    ...
    try:
        ...
        if "time_offset = 0" in content:
            ...
            # Also patch the Session class directly
            from pyrogram.session import Session
            Session.time_offset = 60
            logging.info("Also patched Session.time_offset directly")
            
            return True
        else:
            logging.warning("Could not find 'time_offset = 0' in session.py")
            
            # Try to patch the Session class directly anyway
            from pyrogram.session import Session
            Session.time_offset = 60
